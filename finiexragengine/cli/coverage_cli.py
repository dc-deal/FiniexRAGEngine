"""CLI entry point: corpus coverage per symbol query (ISSUE_24 groundwork).

The retrieval floor profile is a different report and has its own command,
`floor_profile_cli` (ISSUE_104).

**This spends**: an uncached query is embedded on the way (ISSUE_19), which is why it has no
HTTP route and no catalog entry — an external caller must not be able to cause spend. A
console run is the operator's own, deliberate spend.
"""
import argparse
import os
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description='Corpus coverage per symbol query: how well the corpus covers what each '
                    'symbol asks for')
