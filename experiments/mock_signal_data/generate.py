...
# Mirrors the real `crypto_sentiment` constellation. A symbol the live pipeline scores but the mock
# omits is a scenario that cannot be backtested at all — DOTUSD was missing here while the archive
# carried it, so keep this list and configs/pipelines/crypto_sentiment.json in step.
DEFAULT_SYMBOLS = ['BTCUSD', 'ETHUSD', 'SOLUSD', 'ADAUSD', 'XRPUSD', 'DASHUSD', 'LTCUSD', 'DOTUSD',
                   'ETHEUR']
# The real `forex_macro_sentiment` constellation, for `--symbols "$(...)"` convenience.
FOREX_SYMBOLS = ['EURUSD', 'GBPUSD', 'USDJPY', 'AUDUSD', 'EURGBP', 'NZDUSD', 'USDCAD', 'USDCHF']
NAME = {
    ...
    'AUDUSD': 'Australian Dollar', 'EURGBP': 'Euro / Sterling', 'NZDUSD': 'New Zealand Dollar',
    'USDCAD': 'Canadian Dollar', 'USDCHF': 'Swiss Franc',
}
...
SCHEMA_VERSION = '2.0'
# The generator's own version, folded into the config fingerprint below. Without it two weeks
# generated by different builds of THIS file share a fingerprint while differing in what they
# contain — the same byte-identical-provenance trap that made `data_origin` necessary, one level
# down. Bump it whenever the emitted shape changes.
GENERATOR_VERSION = '3'
...
EXTRA_MIN_S, EXTRA_MAX_S = 60.0, 540.0     # where inside the bar the unscheduled pass STARTS
# How long an unscheduled pass runs. Usually seconds, like any pass — but a slow one is what
# produces the seq/evidence inversion (ISSUE_9 RC-4): it started earlier, so its evidence is older,
# yet it commits after the next scheduled pass and therefore carries the HIGHER seq. A consumer
# that reads "newer seq" as "newer analysis" mistakes it for a reversal. The bound is the engine's
# own `pass_timeout_seconds` (300 s): past that the real pass is abandoned and produces nothing.
LONG_EXTRA_RATE = 0.30                     # of unscheduled passes, how many run long
LONG_EXTRA_MIN_S, LONG_EXTRA_MAX_S = 120.0, 295.0
# Between an article's publication and our fetch. The draw is log-uniform over the range measured on
# the live journal — sub-second (a feed with no publication date, where the two coincide) to 8.5 h —
# so the mass sits in minutes with a tail into hours. Note the realised spread is NARROWER than that
# range: a fetch cannot postdate the retrieval that cited it, and the generator's articles are at
# most ~3 h old, so a full week comes out at roughly 0.4 s to 0.8 h with a median near two minutes.
# Stated because the range and the outcome are different numbers, and only one of them is measured.
FETCH_LAG_MIN_S, FETCH_LAG_MAX_S = 0.4, 30600.0
...
def _sources(symbol: str, when: datetime, k: int) -> list:
    """The articles one row cites, as of `when` — the moment the evidence was gathered.

    Deterministic from symbol+time+index: with a shared k per cycle, every variant cites the exact
    same articles (identical retrieval context).

    `fetched_at` is when *we* pulled the article, which is what `evidence_as_of` is derived from
    (ISSUE_9) — never `published_at`, which the publisher controls and can backdate. It sits
    between publication and `when`, drawn log-uniformly from the lag measured on the live journal,
    and is derived from the article's own digest so a regenerated week is byte-identical.
    """
    refs = []
    ...
    for i in range(k):
        ...
        # Log-uniform in [FETCH_LAG_MIN_S, FETCH_LAG_MAX_S], then clamped so a fetch can never be
        # claimed after the retrieval that cited it.
        unit = int(digest[:8], 16) / 0xFFFFFFFF
        lag_s = FETCH_LAG_MIN_S * (FETCH_LAG_MAX_S / FETCH_LAG_MIN_S) ** unit
        fetched_at = min(published_at + timedelta(seconds=lag_s), when)
...
def _evidence_as_of(refs: list):
    """Newest `fetched_at` across a row's evidence, epoch-ms — None when the row has none.

    Derived exactly as the engine derives it (`symbol_evaluator._evidence_as_of`), so a consumer
    can assert the same invariant against the fixture and against live data.
    """
    stamps = [r.fetched_at for r in refs if r.fetched_at is not None]
...
def _make_envelope(pipeline_id, status, timestamp, results, metadata, errors, prompt,
                   fingerprint, trigger_reason):
    prompt_id, prompt_version, prompt_hash = prompt
    return AnalysisEnvelope[SentimentResult](
        schema_version=SCHEMA_VERSION,
        pipeline_id=pipeline_id,
        # Top level since schema_version 2.0 (ISSUE_9). It used to be passed to RunMetadata, where
        # Pydantic silently dropped it after the promotion — every line then claimed '' , i.e.
        # "produced before this field existed", which for a freshly generated fixture is a lie.
...
def _render_variant(variant: _Variant, facts: dict, symbols, bar_close, force_error, prompt,
    ...
    if force_error:
        ...
        )   # a failed pass still names why it ran — the reason rides on the envelope
        ...
        return _make_envelope(variant.stream_id, 'error', analysis_at, [], metadata, errors,
                              prompt, fingerprint, trigger_reason)
    ...
    # Retrieval happens at the START of a pass. For a scheduled pass that is the bar close and the
    # distinction is seconds; for a long unscheduled one it is minutes, and it is exactly what makes
    # its evidence older than a pass that committed before it (ISSUE_9 RC-4).
    evidence_at = started_at if started_at is not None else bar_close
    ...
    for sym in symbols:
        ...
        results.append(SentimentResult(
            ...
            sources=(refs := _sources(sym, evidence_at, facts['source_k'][sym])),
            evidence_as_of=_evidence_as_of(refs),
            base_currency=sym[:-3], quote_currency=sym[-3:],       # pair legs (ISSUE_70)
        ))
    errors = []
    if facts['partial']:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help='two runs sharing a seed produce correlated weeks - the same inversion positions even '
             'across different pipelines. Vary it when two streams must be independent samples.')
    ...
    # Envelopes are buffered per stream, then numbered in COMMIT order (ISSUE_9). That order is
    # what the engine's counter produces — it mints immediately before the insert — and it is the
    # only way the seq/evidence inversion below comes out right: a long unscheduled pass started
    # earlier, so its evidence is older, yet it commits later and therefore carries the HIGHER seq.
    # Numbering in generation order instead would quietly hide the very case this fixture exists
    # to exercise.
    pending = {}
    ...
    for i in range(args.cycles):
        ...
        # Each is (start inside the bar, duration): usually seconds, sometimes minutes.
        extras = []
        if not short and rng.random() < EXTRA_ENVELOPE_RATE:
            start_s = EXTRA_MIN_S + rng.random() * (EXTRA_MAX_S - EXTRA_MIN_S)
            duration_s = (LONG_EXTRA_MIN_S + rng.random() * (LONG_EXTRA_MAX_S - LONG_EXTRA_MIN_S)
                          if rng.random() < LONG_EXTRA_RATE
                          else _pass_seconds(rng.random(), PASS_MEDIAN_S['gpt-4o-mini']))
            extras.append((start_s, duration_s))
        for variant in variants:
            ...
            pending.setdefault(variant.stream_id, []).append(envelope)
            ...
            for start_s, duration_s in extras:
                extra = _render_variant(variant, facts, symbols, bar_close, False, prompt,
                                        fixed_offset_s=start_s + duration_s,
                                        trigger_reason='breaking', fingerprint=fingerprint,
                                        started_at=bar_close + timedelta(seconds=start_s))
                pending.setdefault(variant.stream_id, []).append(extra)
    ...
    for stream_id, envelopes in pending.items():
        ...
        for seq, envelope in enumerate(envelopes, start=1):
            ...
            envelope.available_msc_resyncs = 0     # a generated clock never steps back
            envelope.available_msc_max_correction_ms = 0
            evidence = max((r.evidence_as_of for r in envelope.result
                            if r.evidence_as_of is not None), default=None)
            if evidence is not None and previous_evidence is not None and evidence < previous_evidence:
                inversions += 1
            previous_evidence = evidence if evidence is not None else previous_evidence
            # No collector exists yet, so the archive sets `collected_msc = timestamp` — mirror
            # `outcome_exporter.py` exactly, declaration included. Bucketing follows the same key,
            # so a pass that finishes past midnight lands in the next day's file.
            ...
            _sink(stream_id, collected_at).write(json.dumps(line) + '\n')

    for handle in handles.values():
        handle.close()
    for key in sorted(paths):
        print(f'wrote {counts[key]} snapshots ({len(symbols)} symbols, {key}) → {paths[key]}')
    # The number the consumer's RC-4 mitigation needs to be exercisable at all: an envelope whose
    # seq is higher than its predecessor's while its evidence is older.
    #
    # The unit is in the label on purpose. An inversion is a property of the ENVELOPE - the maximum
    # `evidence_as_of` across its rows. Measured PER ROW the same week yields two orders of
    # magnitude more (17 vs 2073 on a crypto week), and none of it is anomalous: a row's stamp
    # legitimately falls between passes when its retrieved set changes - the recency window slides,
    # the similarity floor cuts differently - so a young article dropping out lowers the maximum
    # with nothing wrong. A consumer that builds its down-weighting on the per-row measure ships a
    # filter that fires continuously in normal operation. The Testing IDE nearly did.
    print(f'seq/evidence inversions (per envelope): {inversions}')
    # Reported so nobody has to construct the query that contains the trap. An `error` envelope
    # carries an empty `result` by contract, so it has no rows and no evidence stamp — a
    # contiguity check that filters on "has an evidence stamp" drops exactly these and reports
    # them as seq holes. The Testing IDE measured 201 such "holes" that way, which were the 201
    # error envelopes. **Contiguity is measured over the full set, inversions over the evidenced
    # subset**; doing both in one query is the mistake, and it looks like an answer every time.
    print('status split: ' + ' · '.join(f'{s} {n}' for s, n in sorted(statuses.items())))
